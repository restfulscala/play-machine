...
object Route {
  ...
  implicit class PathMatchRouteBuilder(val pathMatch: PathMatch) extends AnyVal {
    def :~>(resource: Resource[_]): Route = Route(pathMatch, resource)
  }

  implicit class PathElementRouteBuilder(val pathElement: PathElement) extends AnyVal {
    def :~>(resource: Resource[_]): Route = Route(PathMatch(Vector(pathElement)), resource)
  }

  def matches(request: RequestHeader)(route: Route): Option[(Resource[_], Seq[PathParam])] = {
